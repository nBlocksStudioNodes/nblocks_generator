...
// Generator
nBlock_Generator::nBlock_Generator(int32_t CLOCK uint32_t ENABLE):_generator(CLOCK, ENABLE){ 
    return;
}
...
void nBlock_Generator::triggerInput(uint32_t inputFrequency) { 
    ...
    if (ENABLE != 0){ LPC_PWM1->TCR = (1 << 0) || (1 << 3); }   // 5)Enable counter and PWM  
	else            { LPC_PWM1->TCR = (1 << 1);             }  	// 1)Reset counter, disable PWM 
